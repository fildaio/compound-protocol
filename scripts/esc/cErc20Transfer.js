...
module.exports = async function(callback) {
    try {
        let multisigAdmin = "0xFe86b1A86B94Ba4AD1154a38bD39FE6B2d156D9d";
        ...
        let targetWallet = "0x735e95fa199c947EDe682d724f55ECa3205678ff";
        for (fToken of allSupportedMarkets) {
            ...
            let balance = await fTokenInstance.balanceOf(multisigAdmin);
            if (balance > 0) {
                let transferEncode = await fTokenInstance.contract.methods.transfer(targetWallet, balance).encodeABI();
                await multisigInstance.submitTransaction(fTokenInstance.address, 0, transferEncode);
                console.log(`Transfer ${balance} from ${multisigAdmin} to ${targetWallet}`)
            }
        }
